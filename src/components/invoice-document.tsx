...
export function InvoiceDocument({
  ...
  return (
    <article className="mx-auto w-full max-w-[210mm] bg-white p-3 text-[12px] leading-snug text-foreground print:max-w-none print:p-0 print:text-[11px] print:leading-snug print:text-black">
      <section className="mx-auto min-h-[297mm] w-[190mm] max-w-full bg-white px-7 py-6 print:flex print:min-h-[267mm] print:w-full print:flex-col print:px-0 print:py-0">
        <header className="border-b-4 border-primary pb-4 print:border-b-2 print:border-black print:pb-3">
          <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_52mm] sm:items-start">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary print:text-[9px] print:text-black">
                Invoice
              </p>
              <h1 className="mt-1 text-2xl font-semibold tracking-tight text-foreground print:text-[23px] print:text-black">
                {documentTitle(invoice)}
              </h1>
              <div className="mt-3 space-y-1 text-[11px] text-muted-foreground print:mt-2 print:text-[10px] print:text-black">
            ...
            <div className="rounded-sm bg-primary p-4 text-primary-foreground print:border print:border-black print:bg-white print:p-3 print:text-black">
              <p className="text-[10px] font-medium uppercase tracking-[0.16em] print:text-[9px]">
                Total
              </p>
              <p className="mt-1 text-2xl font-semibold print:text-[22px]">
                {formatCurrency(invoice.total)}
              </p>
              <dl className="mt-4 grid gap-1 text-[11px] print:mt-3 print:text-[10px]">
        ...
        <section className="mt-5 grid gap-3 print:mt-3 print:gap-2 sm:grid-cols-[minmax(0,1fr)_58mm]">
          <div className="rounded-sm border border-border p-4 print:border-black print:p-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground print:text-[9px] print:text-black">
              Bill To
            </p>
            <h2 className="mt-2 text-base font-semibold print:text-[13px]">
              {tenant.name}
            </h2>
            <div className="mt-2 space-y-1 text-[11px] text-muted-foreground print:text-[10px] print:text-black">
              {lines.map((line) => (
                <p key={line}>{line}</p>
              ))}
        ...
        <table className="mt-5 w-full table-fixed border-collapse text-[11px] print:mt-3 print:text-[10px]">
          <colgroup>
            <col className="w-[9%]" />
            <col className="w-[50%]" />
            <col className="w-[12%]" />
          ...
          <tbody>
            {invoiceRows.map((row) => (
              <tr key={row.key} className="h-9 align-top print:h-7">
                ...
                <td className="border border-border px-2 py-2 print:border-black print:px-1.5 print:py-1.5">
                  ...
                  {row.evidence ? (
                    <p className="mt-1 text-[10px] text-muted-foreground print:text-[9px] print:text-black">
                      หน่วยเริ่มต้น{" "}
                      {formatMeterDate(
            ...
            <tr className="bg-secondary/70 print:bg-white">
              ...
              <td className="border border-border px-2 py-2 text-right text-sm font-semibold print:border-black print:px-1.5 print:py-1.5 print:text-[12px]">
        ...
        <section className="mt-10 grid grid-cols-2 gap-6 text-center font-semibold print:mt-auto print:pt-6 print:text-[11px]">
          <div>
...
function DocMeta({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-3 border-b border-border pb-1 last:border-0 last:pb-0 print:gap-2 print:border-black">
      <dt className="text-[10px] font-medium text-muted-foreground print:text-[9px] print:text-black">
        {label}
      </dt>
      <dd className="text-right text-[11px] font-semibold print:text-[10px]">
        {value}
      </dd>
    </div>
  );
