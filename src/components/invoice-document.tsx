...
export function InvoiceDocument({
  ...
  const blankRows = Array.from({
    length: Math.max(5 - invoiceRows.length, 0),
  });

  return (
    <article className="mx-auto w-full max-w-[210mm] bg-white p-3 text-[12px] leading-snug text-foreground print:p-0 print:text-black">
      <section className="mx-auto min-h-[297mm] w-[190mm] max-w-full bg-white px-7 py-6 print:min-h-0 print:w-full print:px-0 print:py-0">
        <header className="border-b-4 border-primary pb-4 print:border-black">
          <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_52mm] sm:items-start">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary print:text-black">
                Invoice
              </p>
            ...
            <div className="rounded-sm bg-primary p-4 text-primary-foreground print:border print:border-black print:bg-white print:text-black">
        ...
        <section className="mt-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_58mm]">
          <div className="rounded-sm border border-border p-4 print:border-black">
            <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground print:text-black">
              Bill To
            </p>
            <h2 className="mt-2 text-base font-semibold">{tenant.name}</h2>
            <div className="mt-2 space-y-1 text-[11px] text-muted-foreground print:text-black">
              {lines.map((line) => (
                <p key={line}>{line}</p>
              ))}
          ...
          <div className="grid gap-2 rounded-sm border border-border bg-secondary/50 p-4 print:border-black print:bg-white">
        ...
        <table className="mt-5 w-full table-fixed border-collapse text-[11px]">
          ...
          <thead>
            <tr className="bg-foreground text-background print:bg-white print:text-black">
              <th className="border border-foreground px-2 py-2 text-left print:border-black">
                ลำดับ
              </th>
              <th className="border border-foreground px-2 py-2 text-left print:border-black">
                รายการ
              </th>
              <th className="border border-foreground px-2 py-2 text-right print:border-black">
                จำนวน
              </th>
              <th className="border border-foreground px-2 py-2 text-right print:border-black">
                ราคา/หน่วย
          ...
          <tbody>
            {invoiceRows.map((row) => (
              <tr key={row.key} className="h-9 align-top">
                ...
                <td className="border border-border px-2 py-2 text-right print:border-black">
                  {row.quantity}
                </td>
                <td className="border border-border px-2 py-2 text-right print:border-black">
                  {row.unitPrice}
                </td>
                <td className="border border-border px-2 py-2 text-right font-semibold print:border-black">
                  {row.amount}
                </td>
              </tr>
            ))}
            {blankRows.map((_, index) => (
              <tr key={`blank-${index}`} className="h-7">
            ...
            <tr>
              <td
                ...
                rowSpan={3}
              />
              <td className="border border-border px-2 py-2 font-semibold print:border-black">
                รวมเป็นเงิน
              </td>
              <td className="border border-border px-2 py-2 text-right font-semibold print:border-black">
            ...
            <tr className="bg-secondary/70 print:bg-white">
              ...
              <td className="border border-border px-2 py-2 text-right text-sm font-semibold print:border-black">
        ...
        {meterEvidence.length ? (
          <section className="mt-5 grid gap-3">
            <div className="flex items-center justify-between gap-3 border-b border-border pb-2 print:border-black">
              <h2 className="text-sm font-semibold">หลักฐานรูปมิเตอร์</h2>
        ...
        <section className="mt-10 grid grid-cols-2 gap-6 text-center font-semibold">
          <div>
            <div className="mx-auto mb-2 w-40 border-t border-foreground print:border-black" />
...
function DocMeta({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-3 border-b border-border pb-1 last:border-0 last:pb-0 print:border-black">
      <dt className="text-[10px] font-medium text-muted-foreground print:text-black">
        {label}
      </dt>
      <dd className="text-right text-[11px] font-semibold">{value}</dd>
    </div>
  );
}
...
function MeterEvidenceBlock({ evidence }: { evidence: InvoiceMeterEvidence }) {
  ...
  return (
    <div className="grid gap-3 rounded-sm border border-border p-3 print:border-black md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_42mm]">
      <EvidenceImageBox
        title="เดือนก่อน"
        date={previousDate}
      ...
      />
      <div className="grid content-start gap-2 rounded-sm bg-secondary/60 p-3 text-[11px] print:border print:border-black print:bg-white">
        ...
        />
        {evidence.warning ? (
          <p className="rounded-sm bg-destructive/10 px-2 py-1 text-destructive print:border print:border-black print:bg-white print:text-black">
            {evidence.warning}
          </p>
        ) : null}
      </div>
...
function EvidenceImageBox({
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="mt-2 flex aspect-[4/3] items-center justify-center overflow-hidden rounded-sm border border-dashed border-border bg-secondary/40 print:border-black print:bg-white">
        {imageUrl ? (
          <img
            src={imageUrl}
          ...
          <p className="px-3 text-center text-[11px] text-muted-foreground print:text-black">
...
function EvidenceLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 border-t border-border pt-2 print:border-black">
